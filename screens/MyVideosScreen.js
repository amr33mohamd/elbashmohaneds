 ...
 headerTitleStyle: {
	...
	componentDidMount() {
		...
		AsyncStorage.getItem('id').then((id)=>{
			fetch(Server.dest + '/api/MyVideos?id='+id).then((res)=>res.json()).then((supjects)=>{
									this.setState({
										doneFetches: 1,
										Subjects: supjects
									});
...
onClick() {
  ...
  return (
		<TouchableOpacity
			onPress={() => this.props.navigation.navigate('ViewVideo',{link:restaurant.video,id:restaurant.id})}
			activeOpacity={0.7}
		>
		      <ImageBackground
